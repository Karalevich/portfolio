...
export const ServicePage: ServicePageComponent = () => {
  ...
  const articles = examples?.map(({ image, link, text }, index) => (
    <article className={styles.article} key={index}>
      {image && (
        <div className={classnames(styles.imageWrapper, { [styles.leftImage]: index % 2 !== 0 })}>
    ...
    <section className={styles.servicePage}>
      ...
      <main className={styles.main}>
        <h3 className={styles.projectsTitle}>
          As a Software Engineer I have worked with a wide range of different projects and technologies,
          here you can get acquainted with some of them.
        </h3>
        {articles}
      </main>
      <footer>
